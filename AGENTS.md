...
- On phone-width layouts, the intraday chart must show the complete selected trading session without horizontal scrolling. Use a compact mobile chart coordinate system and move endpoint labels into a two-column touch legend below the plot; keep the desktop endpoint-label layout unchanged.
